...
@HiltWorker
class CreatePendingEventWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result {
        ...
        val pendingCreateEventId = params.inputData.getString(EVENT_ID)
            ?: return Result.failure()
        val pendingCreatedEventEntity = agendaPendingSyncDao
            .getCreatedEventPendingSyncEntity(pendingCreateEventId)
            ?: return Result.failure()
        val event = pendingCreatedEventEntity.event.toEvent()
        return when (val result = remoteAgendaDateSource.createEventSync(event)) {
            is com.pronoidsoftware.core.domain.util.Result.Error -> {
                result.error.toWorkerResult()
            }

            is com.pronoidsoftware.core.domain.util.Result.Success -> {
                agendaPendingSyncDao.deleteCreatedEventPendingSyncEntity(pendingCreateEventId)
                Result.success()
            }
        }
    }
